...
import { hasProAccess, decreaseUserQuota, getUserTier } from '../storage/auth.js';
...
export async function performAIAnalysis(question, renderHistory, isFollowUp = false) {
    try {
        ...
        const tierModeMap = {
            'free': 'simple',      // 免费用户：简化版（600字）
            'monthly': 'standard', // 月度 Pro：标准版（1500字）
            'yearly': 'deep',      // 年度 Pro：深度版（2500字）
            'lifetime': 'expert',  // 终身 Pro：专业版（3000字+专业术语）
            'admin': 'expert'      // 管理员：专业版
        };
        state.selectedMode = tierModeMap[userTier] || 'simple';
...
请在分析中有意识地校正上述偏差模式，提高推演精度。`;
    ...
    // 根据用户层级选择输出格式
    // simple: 免费用户（600字，无专业术语）
    // standard: 月度 Pro（1500字，标准深度）
    // deep: 年度 Pro（2500字，深度分析）
    // expert: 终身 Pro/管理员（3000字+，含专业术语）
    
    const outputFormats = {
        simple: {
            title: '简化版：非专业客户直读指南',
            maxTokens: 1500,
            termBan: '【术语零容忍铁律】以下词汇及其任何变体绝对禁止出现在简化版输出中，包括括号注释也不行：\n体、用、体卦、用卦、体克用、用克体、体生用、用生体、比和、旺、相、休、囚、死、月令、轨一、轨二、双轨矩阵、大胜、大败、假胜、挡灾、对卦、变卦体用、经卦。\n如果需要表达这些概念，必须100%翻译成生活化的白话，绝不允许原词出现。',
            sections: `
### 🎴 【卦象速览】
（用一行简明文字告知用户所得卦象，格式示例："得**泽天夬**卦，第三爻动，变**泽风大过**卦。" 只说卦名和动爻。）

### 🔮 【一语定调】
（用2-3句话给出综合性行动建议。第一句话直接回答用户最关心的：这件事现实上是"可成"、"可成但代价大"、"难成"，还是"难成但拦下未必是坏事"。）

### 📊 【现状与大势】
（用比喻和直白语言讲清楚：你当下处在一个什么样的宏观大环境里？基础底盘如何？只讲整体氛围、基础强弱和大方向。）

### 🔄 【过程与结局】
（事情接下来会遇到什么助力/阻力？最终的客观结果是什么？将动爻爻辞的核心含义用大白话自然融入过程描述中——先引原文，再用白话解释。）

### 💡 【行动建议】
（以爻辞为核心锚点给出1-2条短而稳的建议。优先写成"先稳住什么、先别做什么"。）
...
（指出当前最不该做的一件事或需要回避的风险。）`
        ...
        standard: {
            title: '标准版：深度客户指南',
            maxTokens: 2500,
            termBan: '【术语限制】尽量避免使用过于专业的易学术语，如必须使用，需在括号中简单解释。',
            sections: `
### 🎴 【卦象速览】
（说明所得卦象、动爻位置、变卦结果。）

### 🔮 【一语定调】
（先用2-3句话给出综合判断，明确现实成败与行动方向。）

### 📊 【现状与大势】
（分析本卦：你当下处在一个什么样的宏观大环境里？基础底盘如何？用清晰的逻辑讲清楚。）

### 🔄 【过程演变】
（分析变卦：事情接下来会遇到什么助力/阻力？态势如何演变？）

### 🏁 【终局判断】
（分析对卦：最终的客观结果是什么？结合旺衰给出准确判断。）

### 💡 【行动建议】
（以爻辞为核心锚点，给出2-3条具体可行的建议。）

### ⚠️ 【慎行事项】
（指出当前最不该做的一件事或需要回避的风险类型。）
...
（列出相关爻辞原文及白话解释，供用户参考。）`
        ...
        deep: {
            title: '深度版：详细推演报告',
            maxTokens: 3500,
            termBan: '可以使用专业术语，但首次出现时建议简要说明。',
            sections: `
## 🎯 【核心战略决断】
（用最精炼的2-3句话直接回答用户问题。第一句明确"现实判断"，第二句明确"行动判断"。）
...
3. **现实终局（对卦定调）**：依据对卦体用关系与旺衰，宣判现实客观成/败。

## 📜 【第三步：长远方向与义理分析】
1. **战略大背景（读卦辞）**：提取本卦卦辞，描述事件宏观底色与整体走势。
2. **战术行动点（读爻辞）**：解读动爻爻辞，给出原则性的动作方向。
3. **综合吉凶判断**：结合卦象给出整体吉凶评估。

## ⚖️ 【最终判断】
（给出高度凝练的最终结论：一针见血的战略定性与稳妥建议。）`
        },
        
        expert: {
            title: '专业版：易学从业者报告',
            maxTokens: 4500,
            termBan: '使用完整专业术语体系，面向易学从业者。',
            sections: `
## 🎯 【核心战略决断】
（用最精炼的2-3句话直接回答用户问题。第一句先明确"现实判断"，第二句再明确"行动判断"，两者都必须与后文全文一致。）
...
## 📜 【第三步：长远方向与双轨矩阵（轨二）】
1. **战略大背景（读卦辞）**：提取本卦卦辞，描述事件宏观底色与整体走势。
2. **战术行动点（读爻辞）**：在卦辞大背景下，解读动爻爻辞，给出原则性的动作方向与因果定性。
3. **双轨矩阵归类**：将对卦现实结果与卦爻综合吉凶放入矩阵，指明终极象限（大胜/大败/假胜/挡灾）。

## 🎭 【第四步：三联卦象深度解析】
（针对终身 Pro 用户，展开本卦、变卦、对卦的详细互卦分析、六亲关系、神煞参考等专业内容。）
...
（给出高度凝练的最终结论：一针见血的战略定性与稳妥建议。）`
    ...
    const format = outputFormats[mode] || outputFormats.simple;
...
你必须在内部完整执行上述所有分析逻辑（月令校准、三段式推演、双轨矩阵判定）。
${format.termBan}
