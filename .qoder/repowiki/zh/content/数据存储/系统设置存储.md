...
## 更新摘要
**变更内容**
- 新增OpenRouter国际模型支持，包括Claude 3.5 Sonnet、GPT-4o、Gemini 2.0 Flash三个高级国际模型
- 新增OpenRouter API密钥管理功能，支持海外模型聚合平台
- 更新模型注册表以支持Pro用户专属的海外模型
- 设置界面需要扩展以支持新的OpenRouter配置项
- 增强Pro用户权限控制，确保海外模型仅对付费用户开放
...
**更新** 本次更新反映了新增的OpenRouter国际模型支持，包括三个高级国际模型(Claude 3.5 Sonnet、GPT-4o、Gemini 2.0 Flash)和OpenRouter API密钥管理功能。
...
**更新** 新增OpenRouter国际模型支持，包括三个高级国际模型的配置管理。

**章节来源**
- [settings.js:38-123](file://src/storage/settings.js#L38-L123)
...
Store->>Store : 检查OpenRouter配置
Store->>Local : 返回配置含默认值
Store-->>Ctrl : 返回配置含默认值
Ctrl-->>UI : 填充表单
User->>UI : 保存设置
...
- [settings.js:67-110](file://src/storage/settings.js#L67-L110)
- [index.html:795-842](file://index.html#L795-L842)
...
### 设置存储模块（settings.js）
- 提供商配置管理
  - 默认端点：为DeepSeek、Kimi、Qwen、Gemini、SiliconFlow、OpenRouter提供默认端点
  - 配置加载：从localStorage读取配置，若缺失则注入默认端点
  - 内置密钥：当存在内置运营密钥时，自动填充SiliconFlow密钥以提升可用性
  - OpenRouter默认配置：自动注入OpenRouter端点和内置API密钥
- 模型注册与选择
  - 模型注册表：定义可用模型及其提供商映射，包括三个Pro专属海外模型
  - 模型选择：从localStorage读取选中的模型，默认为主线路
  - 权限控制：Pro用户专属模型标记(proOnly)，确保仅付费用户可见
- API密钥验证
  - 任意密钥检测：判断是否存在有效API密钥（内置密钥视为有效）

**更新** 新增OpenRouter国际模型支持，包括三个高级国际模型(Claude 3.5 Sonnet、GPT-4o、Gemini 2.0 Flash)和OpenRouter API密钥管理。
...
HasBuiltIn --> |否| CheckOpenRouter["检查OpenRouter配置"]
FillKey --> CheckOpenRouter
CheckOpenRouter --> HasOpenRouter{"存在OpenRouter配置？"}
HasOpenRouter --> |是| ReturnConfigs["返回配置"]
HasOpenRouter --> |否| InjectOpenRouter["注入OpenRouter默认配置"]
InjectOpenRouter --> ReturnConfigs
ReturnConfigs --> End(["完成"])
```

**图表来源**
- [settings.js:67-97](file://src/storage/settings.js#L67-L97)

**章节来源**
- [settings.js:9-123](file://src/storage/settings.js#L9-L123)

### 设置控制器（settings-controller.js）
- 表单加载与填充
  - 从设置存储加载配置，填充DeepSeek与SiliconFlow的密钥与端点
  - 若端点为空，使用默认端点
  - **更新**：当前控制器仅处理DeepSeek和SiliconFlow配置，OpenRouter配置需要在UI层面扩展
- 保存逻辑
...
**更新** 当前设置控制器仅支持DeepSeek和SiliconFlow配置，需要扩展以支持OpenRouter配置。
...
- 模型选择与权限控制
  ...
  - **更新**：Pro用户专属模型仅在hasProAccess()为true时显示
- 云端历史同步
  - 用户登录后从云端拉取历史并与本地合并，确保数据一致性

**更新** 增强了Pro用户权限控制，确保海外模型仅对付费用户开放。
...
CheckAccess --> |是| ShowModels["显示所有模型含海外模型"]
...
- 设置模态框结构
  - 包含主线路与备用线路的密钥与端点输入框
  - **更新**：需要扩展以支持OpenRouter配置项
  - 保存按钮与警告提示区域

**更新** 设置模态框需要扩展以支持OpenRouter配置项，包括密钥和端点输入框。

**章节来源**
- [modals.js:11-56](file://src/ui/modals.js#L11-L56)
- [dom.js:17-41](file://src/utils/dom.js#L17-L41)
- [index.html:873-917](file://index.html#L873-L917)
...
- 认证存储
  ...
  - 用户注册、登录、会话恢复与登出流程
  - 配额管理：游客配额与VIP用户配额
  - **更新**：hasProAccess()函数用于判断用户是否为Pro用户
- 历史存储
  - 本地localStorage存储用户历史记录
  - 云端同步：异步上传与合并，确保数据一致性
  - 反馈存储：用户反馈记录的本地存储与容量限制

**更新** 新增hasProAccess()函数，用于判断用户是否具有Pro权限，从而控制海外模型的显示。
...
**图表来源**
- [settings.js:67-123](file://src/storage/settings.js#L67-L123)
...
**更新** 新增的OpenRouter国际模型支持进一步增强了系统的国际化能力，通过Pro用户权限控制确保了功能的合理分配。系统在默认值管理、URL验证、配额控制与错误容错方面表现出良好的健壮性。
...
- 配置管理最佳实践
  ...
  - 在生产环境中启用HTTPS端点
  - 使用内置密钥时注意API消费限额
  - **新增**：Pro用户专属模型仅对付费用户开放
  - **新增**：OpenRouter API密钥管理注意事项
