...
**更新** 新增divination-warning警告系统，集成到现有用户界面中，包括输入状态管理、仪式保持机制等用户体验改进。
...
- 主题系统（浅色/深色切换）
- 响应式布局（桌面/移动端）
- **新增** 起卦警示系统（divination-warning）
- **新增** 净心引导系统（ritual-guide）

**更新** 新增divination-warning警告系统和ritual-guide引导系统，增强用户输入体验和仪式感。
...
CheckMeaningful --> |是| ShowWarning["显示起卦警示"]
ShowWarning --> SemiTransparent["半透明提示保持仪式感"]
```

**图表来源**
- [src/main.js:615-674](file://src/main.js#L615-L674)
...
**图表来源**
- [src/main.js:676-693](file://src/main.js#L676-L693)
- [src/index.css:4295-4360](file://src/index.css#L4295-L4360)
...
**更新** 新增起卦警示系统相关API，包括输入状态管理和仪式保持机制。
...
**更新** 新增divination-warning和ritual-guide相关CSS样式定义。
...
- **新增** 输入状态管理：通过 isMeaningfulDivinationQuestion 和 DivinationEngine.parseFromText 实现智能输入提示
- **新增** 仪式保持机制：divination-warning 和 ritual-guide 提供沉浸式用户体验

**更新** 新增输入状态管理和仪式保持机制的最佳实践。
...
**更新** 新增divination-warning和ritual-guide系统与main.js的依赖关系。
...
- **新增** 输入状态管理优化：通过防抖减少频繁的DOM操作
- **新增** CSS动画优化：使用 transform 替代布局属性，提升动画性能

**更新** 新增输入状态管理和CSS动画性能优化建议。
...
**更新** 新增divination-warning和ritual-guide相关故障排查指南。
...
**更新** 新增的divination-warning警告系统和ritual-guide引导系统进一步完善了用户体验，通过智能输入状态管理和仪式保持机制，提升了系统的易用性和沉浸感。建议在扩展新功能时遵循现有API与状态管理模式，确保一致性与可维护性。
